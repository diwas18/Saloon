...
@section('content')
    <div class="container mx-auto">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-3xl font-semibold text-blue-600 transform hover:scale-105 transition-all duration-300">Add New Branch</h1>
        </div>

        <div class="mt-6">
            <form action="{{ route('branches.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-2 gap-6">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="location" id="location" class="w-full p-2 border border-gray-300 rounded mt-2" value="{{ old('location') }}" required>
                        @error('location')
                            <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="contact_number" id="contact_number" class="w-full p-2 border border-gray-300 rounded mt-2" value="{{ old('contact_number') }}" required>
